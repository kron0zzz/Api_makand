...
export const createRentalCut = async (req, res) => {
  try {
    const createRentalCut =
      new CreateRentalCut(rentalCutRepository, orderRepository, order_detailRepository, returnRepository);

    const rentalCut = await createRentalCut.execute(
      req.body
    );
